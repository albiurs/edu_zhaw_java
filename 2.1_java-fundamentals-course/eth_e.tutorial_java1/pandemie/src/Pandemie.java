import terminal.VT;
import terminal.VTerm;

public class Pandemie {

	/*
	 * Deklaration und Initialisierung der Datenfelder 
	 */

    // globale Matrix für das Speichern der Population
    static int[][] population = new int[21][51];
    static VTerm console = VTerm.getInstance(25, 80, "Pandemie", VT.CS_TINY);

    public static void main(String[] args) {
        int i, j; // Laufvariablen für die Schleifen
	
	/* 
	 * Operation
	 */

	    // Die Werte im Array population werden auf 0 gesetzt
	    for (i = 0; i < 21; i++) {
		    for (j = 0; j < 51; j++) {
			    population[i][j] = 0;
		    }
	    }
	    // Ausgabe im Fenster von console (Typ VTerm)
	    for (i = 0; i < 21; i++) {
		    for (j = 0; j < 51; j++) {
			    if (population[i][j] > 0) {
				    console.print("" + population[i][j]);
			    } else {
				    console.print(' ');
			    }
		    }
		    console.println();
	    }
	    console.delay(500); // Warten für 500 Millisekunden
	    // erste Person soll angesteckt werden
	    //population[10][15] = 1;
	    init(); // Aufruf Funktion "init" > zufällige Ansteckung der initialen 3 Personen

	    for (int tage = 0; tage < 30; tage++) {
		    for (i = 1; i < 20; i++) {
			    for (j = 1; j < 50; j++) {
				    // Berechnung der Ansteckung
				    ansteckung(i,j);
			    }
		    }
		    console.clearScreen(); // Löschen des Inhalts von console
		    console.println("Tag " + tage);
		    // Ausgabe im Fenster von console (Typ VTerm)
		    print();
		    console.delay(500);
	    }
	    // schliessen der Konsole mit Enter
	    console.readyToExit(true);
    }


    /* 
     * Hinzufügen der neuen Methoden 
     */

    // Prozedur "init" > zufällige Ansteckung der ersten 3 Peraonen
    public static void init () {
	    int x, y; // lokale Variablen
	    x = (int)(Math.random()*21)+1;
	    y = (int)(Math.random()*51)+1;

    }

    // Methode/Prozedur "ansteckung"
    public static void ansteckung (int x, int y) { 
	    if (population[x][y] == 0 && (population[x-1][y] == 1 || population[x+1][y] == 1 || population[x][y-1] == 1 || population[x][y+1] == 1)) {
		    int zufall;
		    zufall = (int) (Math.random() * 4);
		    if (zufall == 0) {
			    population[x][y] = 1;
		    }
	    }
    }

    // Methode/Prozedur "print" > Ausgabe 
    public static void print() { 
	    int i, j; // Laufvariablen für die Schleifen
	    for (i = 0; i < 21; i++) {
		    for (j = 0; j < 51; j++) {
			    if (population[i][j] > 0) {
				    console.print("" + population[i][j]);
			    } else {
				    console.print(' ');
			    }
		    }
		    console.println();
	    }
    }
}

